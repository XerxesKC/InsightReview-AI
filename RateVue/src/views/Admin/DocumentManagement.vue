<template>
  <div class="admin-document-container">
...
const documentParams = reactive({
  ...
  uploaderId: adminStore.adminInfo.adminId,
  uploaderType: 'admin',
})
...
const showHistory = async () => {
  ...
  try {
    const response = await getDocumentHistory({
      ...
      uploader_type: 'admin',
      uploader_id: adminStore.adminInfo.adminId,
...
.upload-tip { color: var(--rv-text-soft); font-size: 12px; line-height: 1.7; margin-top: 10px; }
...
.file-icon { font-size: 36px; color: var(--rv-primary); }
...
.file-size { font-size: 12px; color: var(--rv-text-soft); }
.remove-btn { font-size: 14px; }
.progress-area { margin-top: 20px; }
.progress-text { font-size: 12px; color: var(--rv-text-soft); margin-bottom: 8px; display: inline-block; }
